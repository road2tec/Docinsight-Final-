...
import { db } from './db';
import type { User, InsertUser, Document, Page, Extraction, ChatMessage } from '@shared/mongo-schema';
...
export class MongoStorage implements IStorage {
  ...
  async getDocument(id: string): Promise<Document | null> {
    return this.documents.findOne({ _id: id as any });
  }
  ...
  async createDocument(doc: Partial<Document>): Promise<Document> {
    ...
    const result = await this.documents.insertOne({ ...doc, _id: newId } as any);
    return { ...doc, _id: newId } as Document;
  }
  ...
  async deleteDocument(id: string): Promise<void> {
    ...
    await Promise.all([
      this.documents.deleteOne({ _id: id as any }),
  ...
  async createPage(page: Partial<Page>): Promise<Page> {
    const newId = new ObjectId().toString();
    const result = await this.pages.insertOne({ ...page, _id: newId } as any);
    return { ...page, _id: newId } as Page;
  }
  ...
  async createExtraction(extraction: Partial<Extraction>): Promise<Extraction> {
    const newId = new ObjectId().toString();
    const result = await this.extractions.insertOne({ ...extraction, _id: newId } as any);
    return { ...extraction, _id: newId } as Extraction;
  }
  ...
  async deleteExtraction(id: string): Promise<void> {
    await this.extractions.deleteOne({ _id: id as any });
  }
  ...
  async createChatMessage(message: Partial<ChatMessage>): Promise<ChatMessage> {
    const newId = new ObjectId().toString();
    const result = await this.chatMessages.insertOne({ ...message, _id: newId } as any);
    return { ...message, _id: newId } as ChatMessage;
  }
  ...
  async getReportsData(userId: string): Promise<any> {
    ...
    for (const doc of allDocs) {
      ...
      if (analysisExtraction && analysisExtraction.data) {
        ...
        // Count document types from structured data
        if (data.structuredData && data.structuredData.documentType) {
          const type = data.structuredData.documentType;
    ...
    return {
      ...
      topKeywords,
      statusDistribution,
      documentTypeDistribution: Object.entries(documentTypeCounts).map(([type, count]) => ({ type, count })),
    };
  }
}
